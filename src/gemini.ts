...
2. If the user's message contains another Slack ID after the first one, that means the user is referring to another user in the message. For example, "I cannot make it on the 20th, but <@U87654321> can make it". In this case, the user wants to book a lunch for the other user mentioned in the message. In this case, use the second Slack ID when updating the lunch state and when referring to the user in the response. 
   * The user may also be asking about other users' lunch agreements without referring to themselves. For example, "Can <@U87654321> make it on the 20th?" In this case, do not update the lunch state and refer to the user mentioned in the message when responding.
